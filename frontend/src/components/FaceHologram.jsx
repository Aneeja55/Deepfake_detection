...
const CyberHead = ({ isAnalyzing, theme }) => {
  ...
  const hologramMaterial = useMemo(() => new THREE.ShaderMaterial({
    transparent: true,
    blending: THREE.AdditiveBlending, // Default, will be updated based on theme
    ...
    fragmentShader: `
      ...
      void main() {
        ...
        // Boost opacity slightly in light mode since we don't use additive blending
        float alphaMult = 0.8;
        gl_FragColor = vec4(finalColor, intensity * alphaMult);
      }
    `
  }), []);

  // Update blending based on the theme
  // Additive blending looks great on dark backgrounds, but is invisible on white backgrounds!
  useEffect(() => {
    hologramMaterial.blending = isLight ? THREE.NormalBlending : THREE.AdditiveBlending;
    hologramMaterial.needsUpdate = true;
  }, [isLight, hologramMaterial]);
  ...
  useFrame((state) => {
    ...
    // Change colors based on analysis state and theme
    if (isAnalyzing) {
      hologramMaterial.uniforms.baseColor.value.set(isLight ? "#9f1239" : "#4A0033"); // Deep Magenta base
      hologramMaterial.uniforms.glowColor.value.set(isLight ? "#e11d48" : "#FF0055"); // Bright Red/Pink glow
    } else {
      hologramMaterial.uniforms.baseColor.value.set(isLight ? "#0369a1" : "#002244"); // Deep Blue base
      hologramMaterial.uniforms.glowColor.value.set(isLight ? "#0284c7" : "#00E5FF"); // Bright Cyan glow
    }
  });
    ...
    <Float speed={1.5} rotationIntensity={0.2} floatIntensity={0.5}>
      <group ref={groupRef} scale={0.45} position={[0, -0.2, 0]}>
        ...
        <points geometry={faceGeometry}>
          <pointsMaterial 
            color={isAnalyzing ? (isLight ? "#e11d48" : "#FF0055") : (isLight ? "#0284c7" : "#00E5FF")}
            size={isLight ? 0.015 : 0.008} 
            transparent={true} 
    ...
    <div style={{ width: '100%', height: '220px', marginBottom: '16px', position: 'relative' }}>
      ...
      <Canvas camera={{ position: [0, 0, 7], fov: 45 }} gl={{ antialias: true, alpha: true }}>
        ...
        <Suspense fallback={
          ...
          <Center>
            <CyberHead isAnalyzing={isAnalyzing} theme={theme} />
          </Center>
        </Suspense>
      </Canvas>
    </div>
